...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const table2 = new AsciiTable().setHeading('ID','NICK','SCORE','PING').setAlign(2, AsciiTable.RIGHT);
        const table3 = new AsciiTable().setHeading('ID','NICK','SCORE','PING').setAlign(2, AsciiTable.RIGHT);
        const table4 = new AsciiTable().setHeading('ID','NICK','SCORE','PING').setAlign(2, AsciiTable.RIGHT);
        const table5 = new AsciiTable().setHeading('ID','NICK','SCORE','PING').setAlign(2, AsciiTable.RIGHT);
        ...
        await samp(options, (error, query) => {
            if(error){
                ...
                .setDescription('Server is offline. (Run the command again)');
                ...
                if(query['online'] > 0) {
                    if (query['online'] > 100) {
                        ...
                        if(query['online'] > 20){
                            for(var i=20;i<40;i++){
                                if(query['players'][i] !== undefined){
                                    table2.addRow(query['players'][i]['id'],query['players'][i]['name'],query['players'][i]['score'],query['players'][i]['ping']);
                                }
                            }
                        ...
                        if(query['online'] > 40){
                            for(var i=40;i<60;i++){
                                if(query['players'][i] !== undefined){
                                    table3.addRow(query['players'][i]['id'],query['players'][i]['name'],query['players'][i]['score'],query['players'][i]['ping']);
                                }
                            }
                            embed.addFields({ name: '\u200B', value: '```\n'+table3+'```' });
                        }
                        if(query['online'] > 60){
                            for(var i=60;i<80;i++){
                                if(query['players'][i] !== undefined){
                                    table4.addRow(query['players'][i]['id'],query['players'][i]['name'],query['players'][i]['score'],query['players'][i]['ping']);
                                }
                            }
                            embed.addFields({ name: '\u200B', value: '```\n'+table4+'```' });
                        }
                        if(query['online'] > 80){
                            for(var i=80;i<100;i++){
                                if(query['players'][i] !== undefined){
                                    table5.addRow(query['players'][i]['id'],query['players'][i]['name'],query['players'][i]['score'],query['players'][i]['ping']);
                                }
                            }
                            embed.addFields({ name: '\u200B', value: '```\n'+table5+'```' });
                        }
